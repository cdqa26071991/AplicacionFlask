...
def createUser(user,password,name,surname1,surname2,age,genre):
    ...
    query = f"INSERT INTO users (user, password, name, surname1, surname2, age, genre) \
...
@app.route("/routenueva",methods=('GET','POST'))
def routenueva():
    if request.method == ('POST'):
        NuevoDato = request.form
        user=NuevoDato['usuario']
        password=NuevoDato['contrasena']
        name=NuevoDato['nombre']
        surname1=NuevoDato['apellido1']
        surname2=NuevoDato['apellido2']
        edat=NuevoDato['edad']
        genre=NuevoDato['genero']
        userData = createUser(user,password,name,surname1,surname2,edat,genre,)
        
        if userData == False:
            return render_template("results.html",login=False)
        else:
            return render_template("results.html",login=True,userData=userData)
    return "SIG IN FUNCIONA"
